...
**gravitation period** can pull Tpref towards the starting temperature
and inflate Tbreadth, avoidance percentiles, core-temperature variation,
and time near limits.
...
`grav_time` is a duration in hours. With `exclude_acclimation = TRUE`,
it is added to the start of the dynamic period; with `FALSE`, it is
added to the start of the complete recording. When
`exclude_start_minutes` is also supplied, the later of the custom start
and the gravitation cutoff is used.
...
These values describe **settled thermal behaviour**. Distance,
shuttling, and occupancy can either cover the complete selected period
or only the settled period, depending on the question:
...
- `Tavoid_lower` and `Tavoid_upper` are lower and upper percentiles of
  the selected `core_T` distribution.
...
- `Tbreadth` describes how far apart the temperatures experienced by the
  fish were. It is the average absolute difference between temperatures
  at two randomly selected observations from the analysed period.

### What does Tbreadth mean?

Imagine choosing two moments from the analysed part of the trial and
asking how different the fish’s core temperature was. Repeat that for
every possible pair and take the average. That average is `Tbreadth`.
...
boundaries, and does not depend on histogram bin size. It summarises
...
Here the thermal distribution metrics are calculated after gravitation,
while distance, shuttling, and occupancy retain the complete dynamic
period. The output records whether gravitation was valid and the start
time used for each metric group. When a breakpoint cannot be estimated,
`gravitation_failure = "warn"` records `NA` for metrics that require a
post-gravitation window rather than silently using the full trial.
...
it is not already present. It also cannot determine retrospectively
whether gravitation was excluded. The summary file should therefore
retain columns or accompanying metadata that record the calculation
window used.
